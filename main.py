...
# ════════════════════════════════════════════
DEFAULT_CONFIG = {
    "bot_token":"8538560833:AAEAxF39-5HEfhvkGzD-fCVumALG5NvnHzA",
    "admin_ids":  [8755093471],
    "channel_username":  "meruemizuu",
    "locked": False,
    "global_limit": None,
    "vip_limit": None,
...
async def cmd_start(update,context):
    ...
    bot_name = cfg.get("bot_name","Vince Codm Checker Bot")
...
async def cmd_buy(update, context):
    ...
    tg = update.effective_user; cfg = load_config()
    bot_name = cfg.get("bot_name","Vince Codm Checker Bot")
    gcash_num  = cfg.get("gcash_number","09910411990")
    gcash_name = cfg.get("gcash_name","R.B.")
...
async def cmd_batchkey(update, context):
    ...
    for _ in range(count):
        k=f"VINCE-{_uuid3.uuid4().hex[:8].upper()}-{_uuid3.uuid4().hex[:4].upper()}"
...
async def on_callback(update,context):
    ...
    if data == "gcash_buy":
        cfg2 = load_config()
        gcash_num  = cfg2.get("gcash_number","09910411990")
        gcash_name = cfg2.get("gcash_name","R.B.")
    ...
    if data.startswith("gcash_approve:"):
        ...
        new_key = f"VINCE-{_th(4).upper()}-{_th(4).upper()}"
        keys = load_keys()
...
async def _show_admin_panel(msg, cfg):
    ...
    uptime_s5=int(time.time()-_railway_start); h5,m5=uptime_s5//3600,(uptime_s5%3600)//60
    bot_name5=cfg.get("bot_name","Vince Codm Checker Bot")
...
async def _handle_admin_text_input(update, context, uid, tg, cfg, action, text):
    ...
    if action.startswith("genkey_"):
        ...
        exp=compute_expiry(dtype,dval)
        import uuid as _uuid
        key=f"VINCE-{_uuid.uuid4().hex[:8].upper()}-{_uuid.uuid4().hex[:4].upper()}"
        dd={"hours":f"{dval}h","days":f"{dval}d","months":f"{dval}mo","lifetime":"Lifetime"}[dtype]
        keys=load_keys()
    ...
    if action=="batchkey":
        ...
        dd={"hours":f"{dval}h","days":f"{dval}d","months":f"{dval}mo","lifetime":"Lifetime"}[dtype]
        for _ in range(count):
            k=f"VINCE-{_uuid2.uuid4().hex[:8].upper()}-{_uuid2.uuid4().hex[:4].upper()}"
            keys[k]={"max_users":mu,"used_by":[],"duration_type":dtype,"duration_val":dval,
                     "expires_at":exp,"created_at":datetime.now().isoformat(),"created_by":tg.id}
            generated.append(k)
        save_keys(keys)
...
def _admin_status_text(cfg, users):
    ...
    return (f"{pe(3)} <b>Admin Panel</b> — Vince Codm Checker Bot {pe(3)}\n"
            f"{pe_sep()}\n"
